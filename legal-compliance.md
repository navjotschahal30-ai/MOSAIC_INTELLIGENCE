...
Every `/api/chat` response has a standard disclaimer appended server-side (not generated by Claude, so it's deterministic and can't be dropped by the model):
...
Every `/api/chat` call logs a structured line via `console.log('[compliance]', ...)` with `userType`, the searched address, `solicitationCompliant`, `solicitationFlags`, and confirmation the disclaimer was appended. This is process-log-only today (stdout), not a persisted audit trail — see gaps.

## 4. Disclaimer text

Single source of truth: `generateDisclaimer()` in `core/compliance.js`. See [3.7](#37-valuation-disclaimers) for the current text.
