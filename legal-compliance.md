...
The disclaimer is no longer appended per-message (it was originally, but that cluttered every chat bubble). It's now shown once, site-wide, in a footer below the chat input — fetched from `GET /api/disclaimer`. **Updated to a short line + "Legal" modal split** so the footer stays unobtrusive while still surfacing brokerage identity, CREA trademark attribution, and non-solicitation language that were previously entirely absent from the UI. Both are sourced from `core/compliance.js` so there's a single canonical copy rather than duplicated text in the frontend.

Short line (`generateDisclaimer()`), shown directly in the footer:

> This chat is AI-generated using TRREB MLS® data and is not professional advice, an appraisal, or a CMA.

Full text (`generateLegalText()`), shown in the "Legal" modal (`LegalModal.jsx`) opened from a link next to the short line:

> Mosaic Real Estate Intelligence is an AI assistant built for Navjot Singh, REALTOR® — Team Mosaic, eXp Realty, Brokerage. Responses are generated by artificial intelligence, may contain errors, and should be independently verified; this is not a formal appraisal, Comparative Market Analysis (CMA), or professional advice. Property information is sourced from TRREB MLS® data, is deemed reliable but not guaranteed, and is subject to change without notice. Not intended to solicit properties currently listed for sale, or buyers/sellers already under contract with another brokerage. The trademarks REALTOR®, REALTORS®, and the REALTOR® logo are controlled by The Canadian Real Estate Association (CREA) and identify real estate professionals who are members of CREA. The trademarks MLS®, Multiple Listing Service®, and associated logos are owned by CREA and identify the quality of services provided by real estate professionals who are members of CREA. See our privacy policy for details.

The "privacy policy" phrase (inside the full text) links to `https://navjotchahal.ca/privacy` (`getPrivacyPolicyUrl()` in `core/compliance.js`) — **this is a placeholder destination; the page may not exist yet.** Confirm it's live before this ships to real users, since a disclaimer that links to a 404 undermines the point of having one.

**Not verified against the signed Ampre/TRREB vendor agreement.** The exact required MLS data copyright/attribution wording (e.g. a `©`-style TRREB notice) was deliberately left out of the draft above rather than guessed at — see [Section 6](#6-known-gaps--required-legal-verification). The CREA trademark paragraph and "deemed reliable but not guaranteed" phrasing are common industry-standard boilerplate, not verified citations.
...
Single source of truth: `generateDisclaimer()` (short footer line), `generateLegalText()` (full text, shown in the "Legal" modal), and `getPrivacyPolicyUrl()`, all in `core/compliance.js`, served via `GET /api/disclaimer` and rendered in `ChatBox.jsx` / `LegalModal.jsx`. See [3.7](#37-valuation-disclaimers) for the current text and the privacy-link caveat.
...
- **The full legal text (`generateLegalText()`, [3.7](#37-valuation-disclaimers)) does not include a TRREB-specific MLS data copyright/attribution notice** (e.g. a `©`-style notice with specific required wording) — deliberately left out rather than guessed at, since the signed Ampre/TRREB vendor agreement (the actual source of that required wording) has not been reviewed. The CREA REALTOR®/MLS® trademark paragraph and "deemed reliable but not guaranteed" language are common industry boilerplate, included on that basis, not as verified citations.
